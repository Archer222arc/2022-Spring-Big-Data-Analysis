...
for i=1:length(n_list)
    ...
    %% semilogy
        ...
        title('CDP dataset, n='+string(n)+', L='+string(l));
        ...
        saveas(gcf, './fig/CDP+'+string(i)+string(j)+'.png');
    end
end
%% make table
for i = 1 : 3
...
label.col = [];
for j = 1 : length(m_list)
label.col = [label.col,dollar("L="+string(L_list(j)))];
end
opt.H = "None";
% time
if i == 3
    opt.caption = "Relative error after $3000$ iterations by Wirtinger Flow on dataset CDP";
end
if i == 1
opt.label = "gaussian";
end
opt.filename = "./table/dataset2"+string(i);
